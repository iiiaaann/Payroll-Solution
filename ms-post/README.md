...
Got the file [project]\src\main\java\ch\cpnv\es\payroll\Controllers\EmployeePost.java
...
```bash
mvn -Dtest=EmployeePostTest test
```
...
```docker
docker build -t ms-payroll-employees-post:latest .
```
...
```
docker run -p 8080:8080 --name ms-payroll-employees-post ms-payroll-employees-post:latest
```
