...
Got the file [project]\src\main\java\ch\cpnv\es\payroll\Controllers\EmployeePut.java
...
```bash
mvn -Dtest=EmployeePutTest test
```
...
```docker
docker build -t ms-payroll-employees-put:latest .
```
...
```
docker run -p 8080:8080 --name ms-payroll-employees-put ms-payroll-employees-put:latest
```
